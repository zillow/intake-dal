...
from intake_dal.dal_online import serialize_panda_df_to_str, deserialize_avro_str_to_pandas
...
def test_dal_online_write_read(mock_get: MagicMock, mock_put: MagicMock):
    ...
    assert_frame_equal(
        df,
        deserialize_avro_str_to_pandas(mock_put.call_args[0][0]['avro_rows']),
        check_dtype=False)

    assert avro_str != mock_put.call_args[0][0]['avro_rows']  # not sure why!

    # todo: Why is this not idempotent?
    assert serialize_panda_df_to_str(df, schema=json.loads(cat.metadata['data_schema'][canonical_name])) \
